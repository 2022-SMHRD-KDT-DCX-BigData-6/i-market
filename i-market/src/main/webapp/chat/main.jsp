...
<%@page import="com.smhrd.model.user.t_userinfoDTO"%>
...
<html>
	...
	<body>
		<%
			// session값 가져오기
			t_userinfoDTO loginUser = (t_userinfoDTO)session.getAttribute("loginUser");
			String c_user_id = (String)session.getAttribute("user_id");
			session.setAttribute("user_id", c_user_id);
			if(loginUser != null) {
			System.out.print(loginUser.getUser_id());
			System.out.print(c_user_id);
			}
		%>
		...
		<header id="header" class="alt">
			...
			<%}else { %>
				...
				<a href="ChatRoomService.do">채팅</a>
		...
		<nav id="menu">	
			...
			<ul class="actions vertical">
				...
				<form action="JoinService.do" method="post">
					...
					<li><input type="text" name="user_phone" placeholder="전화번호를 입력하세요" ></li>
					<li><input type="text" name="user_addr" placeholder="집주소를 입력하세요" ></li>
		...
		<!-- Banner -->
			<section id="banner" class="major">
				...
				<header class="major">
					...
					<%}else { %>
					<h1><%=loginUser.getUser_id() %>님 환영합니다</h1>
					<%} %>
				...
				<section class="split">
					...
					<section>
						<div class="contact-method">
							...
							<%}else { %>
							<span><%=loginUser.getUser_phone() %></span>
							<%} %>
					...
					<section>
						<div class="contact-method">
							...
							<%}else { %>
							<span><%=loginUser.getUser_addr() %></span>
							<%} %>
